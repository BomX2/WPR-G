...
namespace WebProjectG.Server.Controllers {
    ...
    public class ValuesController : ControllerBase {
        ...
        public async Task<IActionResult> PutKlant(string id, Gebruiker gebruiker) {
            ...
            try { await _huurContext.SaveChangesAsync(); } catch (DbUpdateConcurrencyException) {
                if (!_huurContext.gebruikers.Any(e => gebruiker.Id == id)) { return NotFound(); } else { throw; }
            }
            return NoContent();
        }
        ...
        public async Task<IActionResult> DeleteKlant(int id) {
            var klant = await _huurContext.gebruikers.FindAsync(id);
            if (klant == null) { return NotFound(); }

            _huurContext.gebruikers.Remove(klant);
            await _huurContext.SaveChangesAsync();
